	...
	<div class="main_block">
		...
		<div class="panel">
			...
			<div class="panel-body">
				...
				<button type="button" class="btn btn-primary green_style" onclick="searchTerms()">
					<i class="fa fa-search"></i> &nbsp;搜索&nbsp;
				</button>
				<button type="button" class="btn btn-primary blue_style add" data-popup-target="addPopup" onclick="addTerm()">
		...
		<table class="table table-hover table-bordered">
			...
			<% for (Term t:terms){%>
				<tr>
					...
					<td><%=t.getAddTime()%></td>
					<td>
						<button class="change" data-popup-target="demoPopup" onclick="updateTerm()"><i class="fa fa-dedent"></i>&nbsp;&nbsp;修改</button>
						<button class="delete" data-popup-target="deletePopup" onclick="delTerm()"><i class="fa fa-times-rectangle-o"></i>&nbsp;&nbsp;删除</button>
					</td>
				</tr>
			<%}%>
			</tbody>
			...
			<script>
				...
				function searchTerms(){
					...
					let termName='';
					let time='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/SearchTerms"
					$.ajax({
						type:"post",
						url:URL,
						data: {id: '', aimName: '', time: ''},
				...
				function addTerm(){
					let termTitle='';
					let termContent='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/AddTerm"
					$.ajax({
						type:"post",
						url:URL,
						data:{termTitle:'测试ajax',termContent:'测试ajax'},
						success:function (data){
							alert('success'+data)
						},error:function (){
							alert('error')
				...
				function updateTerm(){
					...
					let termTitle='';
					let termContent='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/UpdateTerm"
					$.ajax({
						type:"post",
						url:URL,
						data: {id: 1, termTitle: '测试ajax', termContent: '测试ajax'},
						success:function (data){
							alert('success'+data)
						},error:function (){
							alert('error')
