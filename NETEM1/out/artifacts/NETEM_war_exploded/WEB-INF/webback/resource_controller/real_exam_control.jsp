...
<body>
<% List<RealExam> realExams = (List<RealExam>) request.getAttribute("realExams"); %>
	<!-- 右边内容 -->
	...
	<div class="panel">
		...
		<div class="panel-body">
			...
			<button type="button" class="btn btn-primary green_style" onclick="searchRealExams()">
				<i class="fa fa-search"></i> &nbsp;搜索&nbsp;
			</button>
			<button type="button" class="btn btn-primary blue_style add" data-popup-target="addPopup" onclick="addRealExam()">
	...
	<table class="table table-hover table-bordered">
		...
		<% for (RealExam r : realExams) {%>
			<tr>
				...
				<td><%=r.getAddTime()%>
								<td>
									<button class="change" data-popup-target="demoPopup" onclick="updateRealExam()"><i class="fa fa-dedent"></i>&nbsp;&nbsp;修改</button>
									<button class="delete" data-popup-target="deletePopup" onclick="delRealExam()"><i class="fa fa-close"></i>&nbsp;&nbsp;删除</button>
								</td>
							</tr>
		<%}%>
		</tbody>
			...
			<script>
				function searchRealExams(){
					let id='';
					let title='';
					let year='';
					let time='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/SearchRealExams"
					$.ajax({
						type:"post",
						url:URL,
						data: {id: '', title: '', year: '', time: ''},
				...
				function addRealExam(){
					let title='';
					let dataUrl='';
					let year='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/AddRealExam"
					$.ajax({
						type:"post",
						url:URL,
						data: {title: '', year: '', dataUrl: ''},
						success:function (data){
							alert('success'+data)
						},error:function (){
							alert('error')
				...
				function updateRealExam(){
					let id='';
					let title='';
					let dataUrl='';
					let year='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/UpdateRealExam"
					$.ajax({
						type:"post",
						url:URL,
                        data: {id: 1, title: 'ajac', year: 'ajac', dataUrl: 'ajac'},
						success:function (data){
							alert('success'+data)
						},error:function (){
							alert('error')
				...
				function delRealExam(){
					let id='';
					let aimName='';
					let time='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/DelRealExam"
					$.ajax({
						type:"post",
						url:URL,
						data:{},
		...
		<ul class="sidebar-menu">
			...
			<li class="treeview" id="li_active">
				...
				<ul class="treeview-menu" id="open">
					...
					<li><a href="RealExamController" class="green_font"><i class="fa fa-dot-circle-o"
																		   aria-hidden="true"></i> 历年真题</a></li>
