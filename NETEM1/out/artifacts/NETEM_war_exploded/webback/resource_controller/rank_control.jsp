...
<body>
<% List<Information> ranks = (List<Information>) request.getAttribute("ranks"); %>
	<!-- 右边内容 -->
	<div class="main_block">
		...
		<div class="panel">
			...
			<div class="panel-body">
				...
				<button type="button" class="btn btn-primary green_style" onclick="searchRanks()">
					<i class="fa fa-search"></i> &nbsp;搜索&nbsp;
				</button>
				<button type="button" class="btn btn-primary blue_style add"   data-popup-target="addPopup" onclick="addRank()">
		...
		<table class="table table-hover table-bordered">
			...
			<% for (Information r:ranks){%>
				<tr>
					...
					<td><%=r.getAddTime()%></td>
					<td>
						<button class="change" data-popup-target="demoPopup"onclick="updateRank()"><i class="fa fa-dedent" ></i>&nbsp;&nbsp;修改</button>
						<button class="delete" data-popup-target="deletePopup"onclick="delRank()"><i class="fa fa-close" ></i>&nbsp;&nbsp;删除</button>
					</td>
				</tr>
			<%}%>
			</tbody>
			...
			<script>
				function searchRanks(){
					...
					let content='';
					let time='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/SearchRanks"
					$.ajax({
						type:"post",
						url:URL,
				...
				function addRank(){
					...
					let time='';
					let image='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/AddRank"
					$.ajax({
						type:"post",
						url:URL,
						data: {title: '测试ajax', content: '测试ajax', time: '', image: '测试ajax'},
						success:function (data){
							alert('success'+data)
						},error:function (){
							alert('error')
				...
				function updateRank(){
					...
					let content='';
					let image='';
					let URL = "${pageContext.request.contextPath}/BackNETEM/UpdateRank"
					$.ajax({
						type:"post",
						url:URL,
						data: {id: 1, title: 'ajax', content: 'ajax', image: 'ajax'},
						success:function (data,textSt){
							alert('success'+data)
							alert(''+textSt)
		...
		<ul class="sidebar-menu">
			...
			<li class="treeview" id="li_active">
				...
				<ul class="treeview-menu" id="open">
					...
					<li><a href="RankController" class="green_font"><i class="fa fa-dot-circle-o"
																	   aria-hidden="true"></i> 院校排名</a></li>
